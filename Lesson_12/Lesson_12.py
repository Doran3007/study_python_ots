...
# Соединяем Питон и БД
async def init():
    await Tortoise.init(
        db_url="sqlite:///Lesson12.db",
...
# Запрос на получение данных url
...
async def get_data(source: Source) -> tuple:
    async with ClientSession() as session:
        res = await fetch(session, source.url)
    return source.name, res


# Записываем данные с помощью корутин
...
async def run():
    await Tortoise.init(db_url="sqlite:///Lesson12.db", modules={'models': ['models']})
    await Tortoise.generate_schemas()

    # Need to get a connection. Unless explicitly specified, the name should be 'default'
    conn = Tortoise.get_connection("default")
    val = await conn.execute_query_dict("SELECT * FROM user")
    print(val)
